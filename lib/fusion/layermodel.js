define([
    ...
    var Memory = arguments[6];
    var ObjectStoreModel = arguments[7];
    ...
    var LayerModel = declare(null, {
        ...
        createMapGuideLayer: function(mapTag) {
            ...
            request.post(url, {
                ...
                try {
                    ...
                    if (mapTag.Extension[0].Options) {
                        for (var key in mapTag.Extension[0].Options[0]) {
                            options[key] = mapTag.Extension[0].Options[0][key][0];
                        }
                    }
                    deferred.resolve(new OpenLayers.Layer.MapGuide("MapGuide - " + resp.mapTitle, Fusion.config.webtier.url, params, options));
                } catch (e) {
        ...
        /**
         * Saves any necessary state of the map widget to this model. Usually invoked before a map switch
         * @ignore
         */
        dumpMapState: function() {
            this.oLayerOL[i].events.unregister("loadstart", this.mapWidget, this.mapWidget.incrementLayerCounter);
            this.oLayerOL[i].events.unregister("loadend", this.mapWidget, this.mapWidget.decrementLayerCounter);
        },
        /**
         * Applies layers and states in this model to the map widget
         * @ignore
         */
        applyToMap: function() {
            var oMapOL = this.mapWidget.oMapOL;
            var mgLayer = null;
            var iBaseLayerCount = 0;
            for (var i = 0; i < this.oLayerOL.length; i++) {
                this.oLayerOL[i].events.register("loadstart", this.mapWidget, this.mapWidget.incrementLayerCounter);
                this.oLayerOL[i].events.register("loadend", this.mapWidget, this.mapWidget.decrementLayerCounter);
            ...
            if (mgLayer != null) {
                ...
                Fusion.initUnits(data.map.metersPerUnit);
                this.mapWidget.setMetersPerUnit(data.map.metersPerUnit);
                oMapOL.setOptions({ projection: mgLayer.projection });
                if (iBaseLayerCount == 0) {
                    mgLayer.isBaseLayer = true;
                    oMapOL.setBaseLayer(mgLayer);
                }
                this._mapName = mgLayer.params.mapname;
            }
            oMapOL.setOptions({ maxExtent: this.mapExtent });
            this.mapWidget.setExtents(this.mapExtent);
        },
        getMapName: function() {
