...
if (strlen($srcdir) > 0) {
    echo "Reading images from $srcdir\n";
    $files = scandir($srcdir);
    foreach ($files as $f) {
        if (strcmp($f, ".") == 0 || strcmp($f, "..") == 0)
            continue;
        array_push($images, $srcdir.DIRECTORY_SEPARATOR.$f);
    }
}
...
$css = ".fusion-icon { background-image: url('$stripFile'); background-repeat: no-repeat; width: 16px; height: 16px }\n";
$imgObjects = array();
$imgStrip = null;
$w = 0;
$h = 0;
foreach($images as $image) {
    if (file_exists($image)) {
        ...
        $css .= <<<EOT
.fusion-{$info['filename']} { background-position: {$px}px {$py}px; }
...
function printHelp() {
    echo "Usage:\n\n";
    echo "    php imagestrip.php [--vertical | --horizontal] [--append] [--verbose] -css <css file name> -o <image strip name> [-srcdir <imagedir> | <image> ...]\n\n";
